...
# [0.5958569738395876, 0.5106933374517315, 0.5538844365753522, 0.6130172676258403, 0.6512654030641648, 0.63489742114818, 0.6142745221944632, 0.5885898388311868]
...
def loadData(essays, set_id, batch_size):
    ...
    if set_id == 8:
        max_score = 60
    return train_loader, test_loader, min_score, max_score
...
def train(net, topk, set_id, min_score, max_score, train_loader, test_loader, loss_func, optimizer, num_epochs, lr_origin, device):
    ...
    for epoch in range(num_epochs):
        ...
        if test_kappa > max_test_kappa[set_id]:
            ...
            torch.save(net.state_dict(), 'HW3/models_normal/model_' + str(set_id) + '.pt')
...
def experiment(essays, embedding, embed_size, device):
    ...
    for set_id in range(1,9):
        for _ in range(2):
            ...
            net = EssayNet(embedding, embed_size, inception_size, hidden_size, topk).to(device)
